...
class Ticket extends Model
{
  ...
  public function scopeFilter(
    ...
    $query->join('reasons', 'reasons.id', 'tickets.reason_id')
      ...
      ->leftJoin('messages', fn($q) => $q->on('messages.ticket_id', 'tickets.id')
        ->whereRaw('messages.id IN (SELECT MIN(m.id) FROM messages m join tickets t on t.id = m.ticket_id GROUP BY t.id)'))
      // ->leftJoin('hidden_chat_messages', fn($q) => $q->on('hidden_chat_messages.ticket_id', 'tickets.id')
      ...
      ->leftJoin('messages AS last_messages', fn($q) => $q->on('last_messages.ticket_id', 'tickets.id')
        ->whereRaw('last_messages.id IN (SELECT MAX(m.id) FROM messages m join tickets t on t.id = m.ticket_id GROUP BY t.id)'))

...
trait DashboardTrait
{
  ...
  public static function getNewTicketsCountByUsers(): array
  {
    $resolved_tickets = ResolvedTicket::join('users', 'users.id', 'resolved_tickets.new_manager_id')
      ...
      DATE(IFNULL(hcm_s.created_at, hcm.created_at)) AS date,
    ...
    foreach ($filter as $key => $f) {
      $has = array_filter($composed, fn($c) => $c['name'] == $f['name'] && $c['date'] == $f['date'] && $c['year'] == $f['year']);
      if (count($has) > 0) {
        // dd($has);
        // array_values($has)[0]
        $composed[key($has)]['count'] += $f['count'];
      } else {
        array_push($composed, $f);
      }
    }
    // dd($composed);
    // foreach ($filter as $key => $f) {
